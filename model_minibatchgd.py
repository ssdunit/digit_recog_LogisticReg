...
for epoch in range(epochs):
    ...
    for i in range(0,m,m_batch_size):

        X_batch = X_train_shuffled[i:i+m_batch_size]
        Y_batch = y_train_shuffled[i:i+m_batch_size]
    ...
    if (epoch+1)%5 == 0:
        loss = -np.mean(np.sum(Y_batch * np.log(A2 + 1e-15), axis=1))
